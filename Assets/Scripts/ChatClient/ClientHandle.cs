...
public class ClientHandle : MonoBehaviour
{
	...
	public static void PlayerMessage(Packet _packet)
	{
		...
		buffer = _packet.ReadBytes(_packet.UnreadLength());

		string message = System.Text.Encoding.UTF8.GetString(buffer);

		Debug.Log(_id + "   user sended   " + message + "    <- this message.");

		// create message foam.

		bool isOwn = true;
		if (_id != Client.instance.myId) isOwn = false;
		ChattingManager.Instance.CreateMessageBox(isOwn, message);
	}
