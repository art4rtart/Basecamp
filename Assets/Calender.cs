...
public class Calender : MonoBehaviour
{
	...
	public Color todayColor1;
	public Color todayColor2;
	...
	public class Day
	{
		...
		/// Constructor of Day
		/// </summary>
		public Day(int dayNum, Color dayColor, GameObject obj, Color defaultColor, Color emptyColor)
		{
			this.dayNum = dayNum;
			this.obj = obj;
	...
	void UpdateCalendar(int year, int month)
	{
		...
		currDate = temp;
		MonthAndYear.text = temp.Year.ToString() + " 년 " + temp.ToString("MMMM");
		...
		if (days.Count == 0)
		{
			for (int w = 0; w < 6; w++)
			{
				for (int i = 0; i < 7; i++)
				{
					...
					{
						newDay = new Day(currDay - startDay, emptyColor, weeks[w].GetChild(i).gameObject, defaultColor, emptyColor);
					}
					else
					{
						newDay = new Day(currDay - startDay, defaultColor, weeks[w].GetChild(i).gameObject, defaultColor, emptyColor);
					}
					days.Add(newDay);
				}
			}
		...
		else
		{
			for (int i = 0; i < 42; i++)
			{
				...
				{
					days[i].UpdateColor(todayColor2);
				}
				else
				{
					days[i].UpdateColor(todayColor);
				}
		...
		{
			days[(DateTime.Now.Day - 1) + startDay].UpdateColor(todayColor2);
		}
